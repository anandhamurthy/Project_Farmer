...
public class MultiDaysAdapter extends RecyclerView.Adapter<MultiDaysAdapter.CropHolder> {

    private Context mContext;
    private List<MultiDays> mMultiDays;

    private ProgressDialog mProgressDialog;

    private FirebaseFirestore db = FirebaseFirestore.getInstance();

    public MultiDaysAdapter(Context context, List<MultiDays> multiDays){
        mContext = context;
        mMultiDays = multiDays;
    }

    @Override
    public CropHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.single_multi_weather_layout, parent, false);
    ...
    public void onBindViewHolder(@NonNull final CropHolder holder, final int position) {
        ...
        final MultiDays multiDays = mMultiDays.get(position);

        holder.Temperature.setText(String.format(Locale.getDefault(), "%.0f°", multiDays.getTemp().getMax()));
        holder.Weather_Icon.setAnimation(getWeatherAnimation(Long.valueOf(multiDays.getWeatherItems().get(0).getId())));
        Calendar calendar = Calendar.getInstance(TimeZone.getDefault());
        calendar.setTimeInMillis(multiDays.getDt() * 1000L);
